...
const ChildCountBadge = styled.div `
  ...
  text-align: center;
`;
const Node = ({ node, isSelected, isFocused }) => {
    ...
    const handleExpandCollapse = useCallback((event) => {
        event.stopPropagation();
        if (node.isExpanded) {
    ...
    return (_jsxs(NodeContainer, { node: node, isSelected: isSelected, isFocused: isFocused, theme: useUIStore.getState().ui.theme, onMouseDown: handleMouseDown, style: { cursor: isDragging ? 'grabbing' : 'pointer' }, children: [_jsxs(NodeContent, { children: [node.key && _jsx(NodeKey, { children: node.key }), isEditing ? (_jsx("input", { ref: editRef, type: "text", value: editValue, onChange: (e) => setEditValue(e.target.value), onBlur: handleEditSubmit, onKeyDown: (e) => {
                            if (e.key === 'Enter')
                                handleEditSubmit();
                            if (e.key === 'Escape')
                                setIsEditing(false);
                            ...
                            width: '80%',
                            outline: 'none',
                        } })) : (_jsx(NodeValue, { children: displayValue })), _jsx(NodeType, { children: nodeTypeLabel })] }), hasChildren && (_jsx(ChildCountBadge, { children: node.children.length })), canExpandCollapse && (_jsx(ExpandCollapseButton, { onClick: handleExpandCollapse, title: node.isExpanded ? 'Collapse' : 'Expand', children: node.isExpanded ? '−' : '+' }))] }));
};
export default Node;
