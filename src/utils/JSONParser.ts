import { JSONNode, NodeType, Position, DocumentState, ConnectionState } from '@/types';
import { NodeModel } from './NodeModel';

export class JSONParser {
  static parseJSON(jsonString: string, documentId: string = 'default'): DocumentState {
    try {
  ...
  static serializeJSON(document: DocumentState): string {
    try {
  ...
  private static parseValue(
    ...
    if (Array.isArray(value)) {
      ...
      const children = value.map((item, i) =>
        this.parseValue(item, null, arrayNode.id, { x: nodePosition.x, y: nodePosition.y }, i)
      );
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      const children = entries.map(([entryKey, entryValue], i) =>
        this.parseValue(entryValue, entryKey, objectNode.id, { x: nodePosition.x, y: nodePosition.y }, i)
      );
  ...
  private static convertNodeToValue(node: JSONNode, nodes: Record<string, JSONNode>): any {
    switch (node.type) {
      case 'object':
        const obj: any = {};
  ...
  static validateJSON(jsonString: string): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
  ...
  static formatJSON(jsonString: string, indentation: number = 2): string {
    try {
      const parsed = JSON.parse(jsonString);
  ...
  static getNodeFromPath(document: DocumentState, path: string): JSONNode | null {
    ...
    for (const part of pathParts) {
      ...
      if (arrayMatch) {
        ...
        if (currentNode.type === 'object') {
          ...
          if (!childNode || childNode.type !== 'array') return null;
          currentNode = childNode;
        }

        if (currentNode.type === 'array') {
          if (index < 0 || index >= currentNode.children.length) return null;
          currentNode = document.nodes[currentNode.children[index]];
        } else {
          return null;
        }
      } else {
        if (currentNode.type !== 'object') return null;
  ...
  static updateNodeFromPath(document: DocumentState, path: string, newValue: any): DocumentState {
    const pathParts = path.split('.').filter(part => part.length > 0);
    const rootNode = Object.values(document.nodes).find(node => !node.parentId);
    ...
    const updateValue = (nodeId: string, parts: string[]): boolean => {
      const node = updatedNodes[nodeId];
      if (!node) return false;

      if (parts.length === 0) {
        const newNode = NodeModel.createNode(
          this.getValueType(newValue),
          newValue,
          node.key,
          node.parentId,
          node.position
  ...
  private static getValueType(value: any): NodeType {
