...
import { JSONNode, NodeType, Position, Dimension, NodeStyle } from '@/types';
...
export class NodeModel {
  ...
  static updateNode(node: JSONNode, updates: Partial<JSONNode>): JSONNode {
    return {
      ...node,
      ...updates,
      metadata: {
  ...
  static addChild(parent: JSONNode, child: JSONNode): JSONNode {
    return {
      ...parent,
  ...
  static removeChild(parent: JSONNode, childId: string): JSONNode {
    return {
      ...parent,
  ...
  static calculateDimensions(type: NodeType, value: any): Dimension {
    const baseWidth = 120;
    const baseHeight = 60;
  ...
  static getDefaultStyle(type: NodeType): NodeStyle {
  ...
  static getDisplayValue(node: JSONNode): string {
    switch (node.type) {
      case 'object':
  ...
  static getNodeTypeLabel(type: NodeType): string {
    switch (type) {
      case 'object':
        return 'Object';
      case 'array':
  ...
  static validateNode(node: JSONNode): string[] {
    const errors: string[] = [];
  ...
  static cloneNode(node: JSONNode): JSONNode {
    return {
      ...node,
      id: uuidv4(),
  ...
  static getNodePath(node: JSONNode, nodes: Record<string, JSONNode>): string[] {
    const path: string[] = [];
    let currentNode: JSONNode | null = node;
  ...
  static findNodeByPath(nodes: Record<string, JSONNode>, path: string[]): JSONNode | null {
